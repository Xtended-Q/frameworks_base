...
package com.android.internal.util.gaming;
...
public class GamingModeController {
    ...
    private void activateGamingMode(boolean enabled) {
        ...
        if (mGamingModeActivated) {
            if(XtendedUtils.IntelligentPerformanceProfileAvailable()){
    ...
    private void deviceSpecificPerfermanceModeHandler(boolean enabled) {
        if(enabled) {
            XtendedUtils.SetLastPerformanceProfileToSettings(mContext, SystemProperties.get(SPECTRUM_SYSTEM_PROPERTY, Resources.getSystem().getString(R.string.config_balanced_profile)));
            SystemProperties.set(SPECTRUM_SYSTEM_PROPERTY, Resources.getSystem().getString(R.string.config_performance_profile));
        } else {
                SystemProperties.set(SPECTRUM_SYSTEM_PROPERTY, XtendedUtils.GetLastPerformanceProfileFromSettings(mContext));
        }
    }
